...
- (void)viewDidLoad {
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc]initWithTitle:@"Finish" style:UIBarButtonItemStyleDone target:self action:@selector(finishClick)];
}
                ...
                [self.RTDelegate uploadImageArray:[_textView.attributedText getImgaeArray] withCompletion:^NSString *(NSArray *urlArray) {
                    ...
                    for (int i=0; i<strArr.count; i++) {
                        ...
                        if (i<urlArray.count) {
                            ...
                            PictureModel *  picture=[urlArray objectAtIndex:i];
...
- (IBAction)finishEditButton:(UIButton *)sender {
...
- (IBAction)fontClick:(UIButton *)sender {
    ...
    if (sender.selected) {
        self.font=25.f;
        [sender setTitle:[NSString stringWithFormat:@"Font %.f",self.font] forState:UIControlStateSelected];
    }
    else
    {
        self.font=DefaultFont;
        [sender setTitle:[NSString stringWithFormat:@"Font %.f",self.font] forState:UIControlStateNormal];
    }
...
-(void)setImageText:(UIImage *)img withRange:(NSRange)range appenReturn:(BOOL)appen
{
    ...
    CGSize targetSize = CGSizeMake(SCREEN_WIDTH , IMAGE_MAX_SIZE * 2);
    if (height / SCREEN_HEIGHT >= width / SCREEN_WIDTH) {
        CGFloat scale =  SCREEN_HEIGHT / height;
        targetSize = CGSizeMake(width * scale - 20, height * scale - 20);
    }else if (height / SCREEN_HEIGHT < width / SCREEN_WIDTH) { // scale width
        CGFloat scale = SCREEN_WIDTH / width;
        targetSize = CGSizeMake(width * scale - 20, height * scale - 20);
    }
    ...
    //Set image size
    imageTextAttachment.imageSize = targetSize;
...
-(NSString*)getHtmlWithImageArray:(NSArray*)images{
    NSLog(@"Image>>>>>");
    return @"";
}
